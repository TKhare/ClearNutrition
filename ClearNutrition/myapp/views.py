...
checkmark = pygame.transform.scale(pygame.image.load("checkmark.png"), (50, 50))
redo = pygame.transform.scale(pygame.image.load("redo.png"), (50, 50))
...
def information1(request):
    ...
    diseases = data[0]
    diseasesInfo = data[1]
    return render(request, 'myapp/diseases.html',context={'diseases':diseases,'diseasesInfo':diseasesInfo})
def home(request):
    return render(request,'myapp/home.html')
def diseases(request):
    return render(request,'myapp/diseases.html')
def upload(request):
    ...
    while not done:
        ...
        while point_count > 0:
            for event in pygame.event.get():
                ...
                draw_win()
            print(points)
            window.blit(checkmark, (width-50, 70))
            window.blit(redo, (width-50, 120))
            pygame.display.update()
            confirmed = False
        while not confirmed:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if width-50 <= pygame.mouse.get_pos()[0] <= width and 70 <= pygame.mouse.get_pos()[1] <= 120:
                        confirmed = True
                        done = True
                    if width-50 <= pygame.mouse.get_pos()[0] <= width and 121 <= pygame.mouse.get_pos()[1] <= 170:
                        confirmed = True
...
def analyze1():
    ...
    stringsList = ["VEGETABLE_OIL","VEGETABLE OIL","Vegetable_Oil","Vegetable_oil","Vegetable oil","Vegetable Oil"]
    print(formatted_list)
    completeDataSet = return_dangerous_info(formatted_list)
    print("\n\n\n\n\n\n\n****************")
    print("**RUNNING CODE**")
    print("***PROCESSING***")
    print("****************")
...
def return_info(ingredients):
    ...
    return dictArr
def return_dangerous_info(ingredients):
    fDD = ["Diabetes","High Blood Pressure", "Low Blood Sugar", "Asthma", "Arthritis", "Chronic Bronchitis"]
    fDI = [
    ["Sugar","High Fructose Corn Syrup","Chocolate","White Flour","Margarine","Molasses","Chocolate"],
    ["Butter", "Sodium Chloride", "Salt", "Margarine", "High Fructose Corn Syrup"],
    ["Canola Oil"],
    ["Eggs", "Peanuts", "Soy", "Fish", "Shrimp"],
    ["Sugar", "White Flour", "Salt", "Corn Oil"],
    ["Caffeine", "Sugar", "Milk", "Butter", "Cheese"]
    ]
    fDI2 = [" "," "," "," "," "," "]
    for indivString in ingredients:
        for i in range(len(fDI)):
            for j in range(len(fDI[i])):
                if(indivString == fDI[i][j]):
                    fDI2[i] += fDI[i][j]
                    fDI2[i] += ", "
    nothing = "No Harmful Ingredient Found"
    for i in range(len(fDI2)):
        if(fDI2[i].isspace()):
            fDI2[i] = nothing+", "
            nothing += " "
    for i in range(len(fDI2)):
    ...
    for i in range(len(fDI)):
        diseases[fDD[i]] = i
        diseasesInfo[fDI2[i]] = i
    dictArr1 = [diseases,diseasesInfo]
    return dictArr1
